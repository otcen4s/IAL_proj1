...
void untilLeftPar ( tStack* s, char* postExpr, unsigned* postLen ) {
  ...
  while(!stackEmpty(s)){ 
    stackTop(s, &top); // save char from top of the stack
    if(top == '('){
      stackPop(s);  // always pop '(' in the end
      break;
    }
    stackPop(s); // pop out that char 
    postExpr[*postLen] = top; // dereferencing postLen to obtain its value and using it as index 
    (*postLen)++; // increment number 
  }
}
...
char* infix2postfix (const char* infExpr) {
  ...
  stackInit(s); //initialization of stack
  ...
  while(*infExpr != '\0'){
    if((*infExpr >= 'a' && *infExpr <= 'z') || (*infExpr >= 'A' && *infExpr <= 'Z') || (*infExpr >= '0' && *infExpr <= '9')){
      postfix[len++] = *infExpr;
    }

    else if(*infExpr == '='){
      // almost copy-paste of function 'untilLeftPar()'
      while(!stackEmpty(s)){ 
        stackTop(s, &postfix[len]);
        stackPop(s);    
        len++;
      }
      postfix[len] = *infExpr;
    ...
    else if((*infExpr == '*') || (*infExpr == '/') || (*infExpr == '+') || (*infExpr == '-') || (*infExpr == '(')){
      doOperation(s, *infExpr, postfix, &len);
    }
